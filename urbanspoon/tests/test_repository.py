...
from urbanspoon.tests.conftest import time_series_factory, spatial_gcm_factory, spatio_temporal_gcm_factory
...
def test_write_colored_maps():
    ...
    with tempfile.NamedTemporaryFile() as outfile:
        repository.write_colored_maps(
            out=f"{outfile.name}.png", format="png", da=fakedata, common_title="sometitle", units="someunits",
            color_bar_range=(0, 1)
        )
...
def test_write_colored_timeseries():
    ...
    with tempfile.NamedTemporaryFile() as outfile:
        repository.write_colored_timeseries(out=f"{outfile.name}.png", format="png", da=fakedata, title="sometitle", units="someunits")
        assert os.path.isfile(f"{outfile.name}.png")
