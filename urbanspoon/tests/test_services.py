...
def test_write_map_periods_average():
    fakedata = xr.Dataset(
        {
            "fakevariable": conftest.spatio_temporal_gcm_factory(
                np.random.rand(4, 361, 721)
            )
        }
    )
    ...
    with tempfile.NamedTemporaryFile() as outfile:
        services.write_map_period_average(
            inputfile=fakedata_storage,
            var="fakevariable",
            color_bar_range=(0, 1),
            outputfile=f"{outfile.name}.png",
            periods=[("1995-01-01", "1995-01-02"), ("1995-01-03", "1995-01-04")],
        )
